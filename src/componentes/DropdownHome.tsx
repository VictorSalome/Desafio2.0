...
export default function BasicMenu({ status, id }: BasicMenuProps) {
    ...
    const setStatus = (newStatus: string) => {
        ...
        const usersLocal: IUser[] = JSON.parse(dadosLocalString as unknown as string);
    ...
    return (
        <div>
            <div onClick={handleClick} style={{ cursor: 'pointer' }}>
                <span style={{ marginRight: '10px', height: '10px', width: '10px', backgroundColor: statusColors, borderRadius: '50%', display: 'inline-block' }}></span>
                {atualStatus}
            </div>
            <Menu
                id="basic-menu"
            ...
            >
                <MenuItem onClick={() => { handleClose(); setStatus('Ativo') }}>Ativo</MenuItem>
                <MenuItem onClick={() => { handleClose(); setStatus('Inativo') }}>Inativo</MenuItem>
                <MenuItem onClick={() => { handleClose(); setStatus('Aguardando ativação') }}>Aguardando Ativação</MenuItem>
                <MenuItem onClick={() => { handleClose(); setStatus('Desativado') }}>Desativado</MenuItem>
            </Menu>
        </div>
    );
}
